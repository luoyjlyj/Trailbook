...
**首版数据接入建议：**支持用户粘贴攻略链接或文本、人工整理的公开信息，以及具备授权或合规接入条件的数据源。对小红书、抖音等平台的具体接入方式，要以平台允许的接口、授权和数据使用条件为准；首版不应把产品可用性押在未经许可的批量抓取上。
...
加入链接/文本导入、地点去重、来源管理、用户收藏和反馈；根据用户修改行为调整推荐偏好，但允许用户查看与重置偏好。
...
以下为初步估算，假设有一支能够并行推进移动端、Web 后台和服务端的 5–7 人产品研发小组，地图/天气服务可正常申请并接入；如内容授权或服务采购尚未确认，周期需另行调整。

| 阶段 | 参考周期 | 主要工作 | 阶段交付与验收重点 |
|---|---:|---|---|
| 需求与数据验证 | 1–2 周 | 选定试点城市、用户场景、数据来源策略；验证地图/天气供应商、权限与费用 | MVP 范围、数据字典、接入清单、明确平台内容使用方式 |
| 产品与技术定稿 | 1–2 周 | 评审四 Tab 结构、行程/地图联动、后台审核流和异常状态 | PRD、交互稿、API/数据结构、技术验证结论 |
| MVP 开发 | 6–8 周 | 开发移动端、Web 后台、后端 API；接入地点库、地图路线、天气和模型 | 从填写计划到查看、修改、保存行程的端到端流程可用 |
| 城市试点与修正 | 2 周 | 小范围真实出行测试；抽查地点与营业信息；分析生成结果和反馈 | 修复主要错误、确认内容维护节奏、决定是否扩城 |
...
| 使用范围 | 需要安装或开通 | 用途 |
|---|---|---|
| **全体五人** | Git；VS Code 或团队统一 IDE；Node.js 24 LTS；固定版本的 pnpm；Docker Desktop/Compose；Chrome；Apifox 或 Postman（二选一） | 拉取代码、安装依赖、运行共享数据库、调试页面与 API；Node.js 24 当前处于 LTS 维护线 |
...
1. 根目录提交 `.nvmrc` 或同类文件，团队统一使用 Node.js 24 LTS；在 `package.json` 固定 pnpm 版本，在 CI 与本地使用同一版本。
...
以下事项适合在需求与数据验证阶段逐项定下，不影响目前对产品形态的理解：

1. 首个试点城市及主要用户群体（如第一次到访、周末短途、美食优先）。
2. 首版允许使用的攻略内容来源、导入形式、授权和保留期限。
3. 地图、天气与大模型服务的候选供应商、费用、配额及使用要求。
4. 移动端技术框架，以及地图 SDK 与该框架的兼容情况。
5. 用户定位、行程数据和偏好信息的授权、保存、删除和分享规则。
6. 运营团队能持续核验的地点数量、类别和更新频率。
